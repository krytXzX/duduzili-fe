...
@Component({
  ...
  template: `
   ...
   <!-- STEP 3: Confirmation -->
        @if (step() === 3) {
            <div class="flex animate-in fade-in slide-in-from-right-4 duration-500 justify-center">
...
export class PromoteStoreModalComponent {
  vendorId = input<string>('');
  close = output<void>();
  promote = output<void>();
  private sellerMonetization = inject(SellerMonetizationService);
  private appToast = inject(AppToastService);
  ...
  confirm() {
    ...
    // Call backend to promote store with selected plan
    this.sellerMonetization.createStorePromotion({ vendorId: this.vendorId() }).subscribe({
      next: () => {
        console.log('Store promotion successful');
        this.close.emit();
      },
      error: (err: any) => {
        console.error('Promotion failed', err);
        this.appToast.show({ message: err?.error?.message || 'Promotion failed', durationMs: 5000 });
      },
      complete: () => {
