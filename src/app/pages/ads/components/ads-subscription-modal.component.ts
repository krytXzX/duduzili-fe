...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-[220] bg-black/20 backdrop-blur-[2px] animate-in fade-in duration-300 md:flex md:p-4" (click)="handleBackdropClick()">
      <div
            ...
            <div class="flex min-h-0 flex-1 flex-col">
              ...
              <div class="min-h-0 flex-1 overflow-y-auto px-4 pb-5">
                <div class="rounded-[18px] bg-[#FAFAFB] p-4 shadow-[inset_0_0_0_1px_rgba(235,237,242,0.9)]">
                  <h2 class="text-[16px] font-semibold tracking-[-0.03em] text-[#1A1C21]">{{ planConfig().name }}</h2>
                  ...
                  <div class="mt-5 space-y-3 border-t border-[#E3E5EA] pt-4 text-[11px] text-[#595E68]">
                  ...
                  <label class="mt-4 flex cursor-pointer items-center gap-2 text-[10px] text-[#464B54]">
                    <input
                      type="checkbox"
                    ...
                    @for (option of billingOptions(); track option.id) {
                      ...
                      >
                        <div class="flex items-start gap-3">
                          <span class="mt-0.5 flex h-4 w-4 items-center justify-center rounded-full border" [class.border-[#6D5AF0]]="selectedBillingId() === option.id" [class.border-[#D7D9E0]]="selectedBillingId() !== option.id">
                          ...
                          <div>
                            <div class="flex items-center gap-2">
                              <p class="text-[12px] font-medium text-[#1A1C21]">{{ option.label }}</p>
                              @if (option.savings) {
                                <span class="rounded-full bg-[#F1F7AA] px-1.5 py-0.5 text-[8px] font-semibold text-[#6A7414]">Save {{ option.savings }}</span>
                              }
                            </div>
                        ...
                        <span class="text-[11px] font-medium text-[#6D5AF0]">For {{ option.total }}</span>
                      </button>
                    }
                  </div>
                </div>
                ...
                <p class="mt-4 text-[9px] leading-4 text-[#6D727C]">
                  By clicking on Subscribe, you agree to the <span class="text-[#6653E4]">Terms of Use</span> and authorize Duduzili to store and charge your payment method.
                </p>
              </div>
            </div>
          } @else {
            <div class="flex flex-1 flex-col items-center justify-center px-5 py-10 text-center">
              <div class="mb-8 flex h-28 w-28 items-center justify-center rounded-full bg-[#DCF4FF]">
                <div class="flex h-16 w-16 items-center justify-center rounded-full bg-[#2CB8F5] shadow-[0_16px_30px_-18px_rgba(44,184,245,0.8)]">
                  <ng-icon name="heroCheck" class="text-3xl text-white"></ng-icon>
                </div>
              </div>

              <h2 class="text-[16px] font-semibold tracking-[-0.03em] text-[#1A1C21]">Subscription successful 🥳</h2>
              ...
              <button
                ...
                class="mt-8 rounded-full bg-[#6653E4] px-10 py-3 text-[12px] font-medium text-white shadow-[0_16px_32px_-18px_rgba(102,83,228,0.9)]"
              >
                Got it
              </button>
            </div>
          ...
          @if (step() === 'review') {
            <div class="grid h-full gap-6 p-8 lg:grid-cols-[minmax(0,1fr)_430px]">
              <section class="min-w-0 pt-2">
                <h2 class="text-[2rem] font-bold tracking-tight text-[#1A1C21]">Review subscription and pay</h2>

                <div class="mt-8">
                  <h3 class="text-[1.05rem] font-semibold text-[#1A1C21]">1. Choose billing cycle</h3>
                  ...
                  <div class="mt-4 space-y-3">
                    @for (option of billingOptions(); track option.id) {
                      ...
                      >
                        <div class="flex items-start gap-3">
                          <span class="mt-1 flex h-5 w-5 items-center justify-center rounded-full border" [class.border-[#6D5AF0]]="selectedBillingId() === option.id" [class.border-[#D7D9E0]]="selectedBillingId() !== option.id">
                            @if (selectedBillingId() === option.id) {
                          ...
                          <div>
                            <div class="flex items-center gap-3">
                              <p class="text-[1rem] font-semibold text-[#1A1C21]">{{ option.label }}</p>
                              @if (option.savings) {
                                <span class="rounded-full bg-[#F1F7AA] px-2 py-0.5 text-xs font-semibold text-[#6A7414]">Save {{ option.savings }}</span>
                              }
                            </div>
                            @if (option.weeklyRate) {
                              <p class="mt-1 text-[0.875rem] font-medium text-[#8C9098]">{{ option.weeklyRate }}</p>
                            }
                          </div>
                        </div>

                        <span class="text-[0.95rem] font-semibold text-[#6D5AF0]">For {{ option.total }}</span>
                      </button>
                    }
                  </div>
                </div>
                ...
                <div class="mt-10">
                  <h3 class="text-[1.05rem] font-semibold text-[#1A1C21]">2. Select your payment method</h3>
                  ...
                  <div class="mt-4 grid gap-3 sm:grid-cols-2">
                    ...
                    >
                      ...
                      <span class="mt-0.5 flex h-5 w-5 items-center justify-center rounded-full border" [class.border-[#6D5AF0]]="selectedPaymentId() === 'wallet'" [class.border-[#D7D9E0]]="selectedPaymentId() !== 'wallet'">
                    ...
                    >
                      <div class="flex items-start gap-3">
                        <ng-icon name="heroGlobeAlt" class="mt-0.5 text-lg text-[#2A2D34]"></ng-icon>
                        <p class="text-[0.95rem] font-medium text-[#1A1C21]">Online</p>
                      </div>
                      <span class="mt-0.5 flex h-5 w-5 items-center justify-center rounded-full border" [class.border-[#6D5AF0]]="selectedPaymentId() === 'online'" [class.border-[#D7D9E0]]="selectedPaymentId() !== 'online'">
                  ...
                  <label class="mt-5 flex cursor-pointer items-center gap-3 text-[0.95rem] font-medium text-[#464B54]">
                    <input
                      type="checkbox"
                      [checked]="isAutoRenewing()"
                      (change)="isAutoRenewing.set(!isAutoRenewing())"
              ...
              <aside class="relative rounded-[28px] bg-[#FAFAFB] p-8 shadow-[inset_0_0_0_1px_rgba(235,237,242,0.9)]">
                ...
                <h3 class="pr-14 text-[2.65rem] font-medium leading-none tracking-tight text-[#1A1C21]">{{ planConfig().name }}</h3>
                ...
                <p class="mt-10 text-[0.875rem] leading-6 text-[#6D727C]">
                  Renews monthly until cancelled. {{ renewalText() }} will be charged. Cancel anytime in Settings.
                  By subscribing, you agree to the <span class="text-[#6653E4]">Terms of Use</span> and authorize Duduzili to store and charge your payment method.
                </p>
              </aside>
            </div>
          } @else {
            <div class="flex h-full flex-col items-center justify-center px-6 py-10 text-center animate-in fade-in zoom-in-95 duration-300">
              <div class="mb-8 flex h-36 w-36 items-center justify-center rounded-[34px] bg-[#DCF4FF] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.65)]">
                <div class="flex h-24 w-24 items-center justify-center rounded-full bg-[#2CB8F5] shadow-[0_16px_30px_-18px_rgba(44,184,245,0.8)]">
                  <ng-icon name="heroCheck" class="text-5xl text-white"></ng-icon>
                </div>
              </div>
              ...
              <p class="mt-4 max-w-[560px] text-[0.95rem] font-medium leading-7 text-[#8E929B]">
                {{ successSubtitle() }}
              </p>
              ...
              <button
                type="button"
                (click)="subscribe.emit()"
                class="mt-8 rounded-full bg-[#6653E4] px-14 py-4 text-[1rem] font-semibold text-white shadow-[0_16px_32px_-18px_rgba(102,83,228,0.9)] transition hover:bg-[#5945DB] focus:outline-none focus:ring-4 focus:ring-[#6653E4]/20"
              >
                Got it
              </button>
            </div>
...
export class AdsSubscriptionModalComponent implements OnDestroy {
    ...
    switch (this.plan()) {
      case 'premium':
        return [
          ...
          { id: 'month', label: '1 Month', weeklyRate: '₦1,200/week', total: '₦4,800', savings: '20%' },
          { id: 'year', label: '1 Year', weeklyRate: '₦750/week', total: '₦39,000', savings: '50%' },
        ];
      case 'enterprise':
        return [
          { id: 'week', label: '1 Week', total: '₦2,000' },
          { id: 'month', label: '1 Month', weeklyRate: '₦1,600/week', total: '₦6,400', savings: '20%' },
          { id: 'year', label: '1 Year', weeklyRate: '₦1,000/week', total: '₦52,000', savings: '50%' },
        ];
      default:
        return [
          { id: 'week', label: '1 Week', total: '₦1,000' },
          { id: 'month', label: '1 Month', weeklyRate: '₦800/week', total: '₦3,200', savings: '20%' },
          { id: 'year', label: '1 Year', weeklyRate: '₦500/week', total: '₦26,600', savings: '50%' },
        ];
    }
  });

  readonly selectedOption = computed(() =>
    this.billingOptions().find((option) => option.id === this.selectedBillingId()) ?? this.billingOptions()[0]
  );
  ...
  readonly successSubtitle = computed(() => {
    return `You have subscribed to ${this.planConfig().name} for ${this.selectedOption().label}. This expires and renews on 7 March, 2026.`;
  });
}
