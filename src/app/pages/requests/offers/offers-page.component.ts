...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NgIcon], // Removed NgOptimizedImage to avoid domain provider issues, fallback to standard img
  providers: [
    provideIcons({
      heroChevronDown,
      heroMagnifyingGlass,
            ...
            <tbody class="divide-y divide-gray-50">
              @for (offer of offers(); track offer.id) {
                <tr class="hover:bg-gray-50/30 transition-colors group">
                  ...
                  <td class="px-6 py-6 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                      <div class="w-10 h-10 rounded-xl bg-gray-100 overflow-hidden border border-gray-100">
                        <img [src]="offer.listing.image" [alt]="offer.listing.name" class="w-full h-full object-cover">
                      </div>
                  ...
                  <td class="px-6 py-6 whitespace-nowrap">
                    <div class="flex items-center gap-2.5">
                      <div class="w-8 h-8 rounded-full bg-gray-100 overflow-hidden border border-gray-100">
                        <img [src]="offer.store.logo" [alt]="offer.store.name" class="w-full h-full object-cover">
                      </div>
                  ...
                  <!-- Date Requested -->
                  <td class="px-6 py-6 whitespace-nowrap">
                    <span class="text-[14px] text-[#1A1C21] font-medium">{{ offer.dateRequested }}</span>
                  </td>
